...
def TotalMatchingRel(G):
    ...
    while it <= 1000:
        ...
        if False and UBk < UB:
            UB = UBk
            mod.addConstr(quicksum(x[v] for v in x) + quicksum(y[e] for e in y) <= UB) 
        else:
                ...
                for i in clique:
                    for j in clique:
                        if i < j:
                            E.append((i,j))            
                if len(clique) % 2 == 1:
                    mod.addConstr(quicksum(y[e] for e in E) <= len(clique)//2)
                else:
                    mod.addConstr(quicksum(x[v] for v in clique) + quicksum(y[e] for e in E) <= len(clique)//2)
            else:
                sep, xb, yb, zb = OddClique(G, xbar, ybar)
                if sep > 0.01:
                    # print('oddclique', sep)
                    mod.addConstr(quicksum(x[v] for v in xb) + quicksum(y[e] for e in yb) <= zb)
                else:
                    ...
                    if ObjC > 0.01 and k > 0:
                        # print('2k3-cycle')
                        mod.addConstr(quicksum(x[c] for c in Cx) + quicksum(y[c] for c in Cy) <= SafeFloor((2*k)/3))
                    else:
                        break
...
def Sep2k3Cycle(G, xbar, ybar):
    ...
    # Length of the cycle
    mod.addConstr(quicksum(x[v] for v in x) + quicksum(y[e] for e in y) == 6*z + 2*w)
    ...
    for v in G.nodes:
       ...
       for e in G.edges(v):
           ...
           A_in.append((j,i))
           A_out.append((i,j))
       mod.addConstr(u[v] + quicksum(f[e] for e in A_in) - quicksum(f[e] for e in A_out) -x[v] == 0)
...
if __name__ == "__main__":
    ...
    logging.info(" v(G) = {:.3f} {:.3f} {:.3f} {:.3f} {} {} {} {}"
                 .format(nu1, nu2, nu3, nu4, it1, it2, it3, it4))
